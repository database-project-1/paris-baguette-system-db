...
이 파일은 본사 운영팀이 매장별 판매 현황과 상품 판매 흐름을 확인한다고 가정하고 만든
분석용 CLI 프로그램입니다. 매장별/지역별 베스트셀러, 브랜드별 매출, 함께 구매된 상품,
카테고리별 매출 비중, 월별 매출 추이 등을 조회할 수 있도록 구성했습니다.
...
주의:
이 프로그램은 판매 데이터를 분석하기 위한 조회 중심의 대시보드입니다.
판매, 재고, 발주 데이터를 직접 수정하지 않으며, 필요한 경우 조회 결과만 CSV 파일로 저장할 수 있습니다.
"""
